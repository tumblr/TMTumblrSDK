...
- (BOOL)singlePasses:(NSString *)title detail:(NSString *)detail logout:(BOOL)logout code:(NSInteger)code needsConsent:(BOOL)needsConsent isConsentBlocking:(BOOL)isConsentBlocking needsAge:(BOOL)needsAge gdprMinimumAge:(NSInteger)gdprMinimumAge authToken:(NSString *)authToken {
    TMAPIErrorFactory *factory = [[TMAPIErrorFactory alloc] initWithErrors:@[
                                                                             @{
                                                                                 @"code" : @(code),
                                                                                 @"title" : title,
                                                                                 ...
                                                                                 @"gdpr_minimum_required_age": @(gdprMinimumAge),
    ...
    return [error logout] == logout && [[error detail] isEqualToString:detail] && [[error title] isEqualToString:title] && [error code] == code && [error needsConsent] == needsConsent && [error isConsentBlocking] == isConsentBlocking && [error needsAge] == needsAge && [error gdprMinimumAge] == gdprMinimumAge && [error authToken] == authToken;
}
...
- (void)testOneObjectCorrectlyTranslatesToModelObjectWithAddedFieldsAllTrue {
    ...
    XCTAssert([self singlePasses:@"some error" detail:@"more about it" logout:NO code:1001 needsConsent:YES isConsentBlocking:YES needsAge:YES gdprMinimumAge:14 authToken:@"abcde12345"]);
}

- (void)testOneObjectCorrectlyTranslatesToModelObjectWithAddedFieldsAllFalse {

    XCTAssert([self singlePasses:@"some error" detail:@"more about it" logout:NO code:1001 needsConsent:NO isConsentBlocking:NO needsAge:NO gdprMinimumAge:16 authToken:@"abcde12345"]);
}

- (void)testOneObjectCorrectlyTranslatesToModelObjectWithAddedFieldsMixedValues {

    XCTAssert([self singlePasses:@"some error" detail:@"more about it" logout:NO code:1001 needsConsent:NO isConsentBlocking:YES needsAge:NO gdprMinimumAge:18 authToken:@"abcde12345"]);
}
